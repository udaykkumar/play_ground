...
static void BM_02_08_2019_Solution(benchmark::State &state)
{
	...
	for (auto _ : state)
	{
		...
		for (decltype(state.range(0)) i = state.range(0);
				i <= (state.range(0) * 2); i++)
			l2.add(i);
		state.ResumeTiming();
...
BENCHMARK(BM_02_08_2019_Solution)
		->Arg(8)->Arg(64)->Arg(512)->Arg(1 << 10)->Arg(8 << 10)->Arg(16 << 10)->Arg(
				32 << 10)->Arg(64 << 10)->Arg(128 << 10)->Arg(256 << 10)->Arg(
				512 << 10);
