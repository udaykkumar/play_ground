...
static int DSTree_CompareData(const void *const TreeData,
		const size_t TreeDataSize, const void *const NewData,
		const size_t NewDataSize, int KeepSimple)
{
	if (KeepSimple)
	{
		...
		const size_t dataSize)
{
	...
	if (data && dataSize > 0)
	{
		...
		{
			int TreeCompare = DSTree_CompareData((*treeHead)->Data.Data,
					(*treeHead)->Data.DataSize, data, dataSize, 1);
			if (TreeCompare < 0)
			{
